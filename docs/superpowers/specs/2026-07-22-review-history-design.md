...
```
  ...
  > **实现变更（记录，不改原文）**：交付版本每轮对象还多返回一个 `consumed`
  > 字段（`true` 当且仅当 wait 脚本已把该轮 `submission-<seq>.json` 改名为
  > `.consumed`），而不是本条最初写的响应形状。这不是可有可无的补充字段：
  > `docVersion` 是提交时**客户端**看到的文档版本，单凭它无法区分"agent 处理过
  > 这一轮但没留回执"与"文档因为别的原因往前走了、这一轮其实还在队列里没被
  > 碰过"——后一种情况下 `docVersion < currentVersion` 一样成立，却不该被当成
  > 已处理。`consumed` 是协议里唯一能证明 agent 真正取走过这一轮的信号（wait
  > 脚本 rename 的那一刻）。§6"轮次级完成态"与下面修订记录的对应条目据此把判定式
  > 改成 `consumed && docVersion < currentVersion`；`skills/cc-redline/assets/js/
  > history.mjs` 的 `roundState` 与 `scripts/tests/history.test.mjs` 都据此实现
  > 与测试。
...
- **取消 `docChanged` 字段 / `partial` 状态 / 三态推导 / 五徽标**：缩小 lockstep
  ...
  > **实现变更（记录，不改原文）**：完成态实际由三个输入推导，而不是本条最初
  > 写的两个——`results`、`consumed`、以及 `docVersion vs currentVersion`。少了
  > `consumed`，单凭 `docVersion` 无法证明 agent 真的处理过这一轮；具体原因、
  > 以及判定式为何要求 `consumed && docVersion < currentVersion`，见 §5
  > "新路由 `GET /api/history`"的实现变更说明。
